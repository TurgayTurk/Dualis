...
Dualis supports MediatR-like request exception contracts, in `Dualis.Pipeline` (mirroring `MediatR.Pipeline`):
...
Multiple handlers can coexist for unrelated exception types/hierarchies and request shapes — each is resolved independently per exception-type level, so a closed handler for one query and an open-generic catch-all handler for a different exception hierarchy don't interfere with each other.

Open-generic, independently-constrained "catch-all" handlers are supported, e.g.:

```csharp
using Dualis.Pipeline;

public sealed class ServerExceptionHandler<TRequest, TResponse, TException>
    : IRequestExceptionHandler<TRequest, TResponse, TException>
    ...
    where TException : ServerException
{
    ...
    {
        state.SetHandled((TResponse)Result.Failure<int>(new Error("ServerExceptionHandler", exception.Message)));
        return Task.CompletedTask;
    }
}
```

### Migrating from MediatR.Pipeline

Existing `IRequestExceptionHandler`/`IRequestExceptionAction` implementations — including open-generic/constrained catch-all handlers like the one above — can be ported by swapping the `using` statement only:
...
No other code changes are needed: type names (`IRequestExceptionHandler<,,>`, `IRequestExceptionAction<,>`, `RequestExceptionHandlerState<TResponse>`) and constraint shapes match MediatR exactly.
